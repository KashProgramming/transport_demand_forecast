...
class TransportForecastService:
    ...
    def prepare_context(
        ...
        # In production, we want to reuse models trained on different files/datasets
        # Set USE_SAMPLE_IN_HASH=false to reuse models across different sample sizes
        # Set USE_DATAFILE_IN_HASH=false to reuse models across different filenames
        use_sample_in_hash = os.getenv('USE_SAMPLE_IN_HASH', 'true').lower() == 'true'
        use_datafile_in_hash = os.getenv('USE_DATAFILE_IN_HASH', 'true').lower() == 'true'
        model_config = self.model_manager.get_config_from_params(
            data_file, effective_sample_size, 
            use_sample_in_hash=use_sample_in_hash,
            use_datafile_in_hash=use_datafile_in_hash
        )
